...
static esp_err_t index_handler(httpd_req_t *req){
    ...
    String page = "";
     ...
     page += "<meta name=\"theme-color\" content=\"#1e1e1e\" />\n";
     page += "<title>e-lab innovations Car</title>\n";
     page += "<style type=\"text/css\"> *, *::before, *::after { box-sizing: border-box; } body { background-color: #1e1e1e; color: #fff; width: 100%; } \n";
     page +="button { position: relative; display: inline-block; box-sizing: border-box; border: none; border-radius: 4px; padding: 0 16px; min-width: 64px;  \n";
     page +=" height: 36px; vertical-align: middle; text-align: center; text-overflow: ellipsis; text-transform: uppercase; color: #fff; background-color: #007acf;  \n";
     page +="box-shadow: 0 3px 1px -2px rgba(0, 0, 0, 0.2), 0 2px 2px 0 rgba(0, 0, 0, 0.14), 0 1px 5px 0 rgba(0, 0, 0, 0.12); font-size: 14px; font-weight: 500;  \n";
     page +=" line-height: 36px; overflow: hidden; outline: none; cursor: pointer; transition: box-shadow 0.2s; text-decoration: none; }  \n";
     page += "</style>\n";
     page += "</head><body>\n";
 page += "<script>var xhttp = new XMLHttpRequest();</script>";
 page += "<script>function getsend(arg) { xhttp.open('GET', arg +'?' + new Date().getTime(), true); xhttp.send() } </script>";
 //page += "<p align=center><IMG SRC='http://" + WiFiAddr + ":81/stream' style='width:280px;'></p><br/><br/>";
 page += "<p align=center><IMG SRC='http://" + WiFiAddr + ":81/stream' style='width:300px; transform:rotate(180deg);'></p>";

// page += "<p align=center>";
// page += "<button style=width:140px;height:40px onmousedown=getsend('pumpon')>Pump ON</button>";
// page += "<button style=width:140px;height:40px onmousedown=getsend('pumpoff')>Pump OFF</button>";
// page += "</p>";
